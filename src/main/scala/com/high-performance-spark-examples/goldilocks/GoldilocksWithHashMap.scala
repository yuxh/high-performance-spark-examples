...
//此方法单独使用是有bug，当有多个分区的时候，mapPartitions会按分区计算,即结果会出现((1.5, 0), 1) ((1.5, 0), 1)，因为这两个可能在不同分区
    ...
    println("aggregatedValueColumnRDD result :")
    aggregatedValueColumnRDD.collect().foreach(println)
    println("end!")
...
//每个分区内，step 1每列的计数的和
   ...
   */
  //tag::hashMap_step3[]
  private def getRanksLocationsWithinEachPart(
      targetRanks: List[Long],
      partitionColumnsFreq: Array[(Int, Array[Long])],
      numOfColumns: Int
  ): Array[(Int, List[(Int, Long)])] = {

    val runningTotal = Array.fill[Long](numOfColumns)(0)

    partitionColumnsFreq.sortBy(_._1).map {
      case (partitionIndex, columnsFreq) =>
        val relevantIndexList = new mutable.MutableList[(Int, Long)]()

        columnsFreq.zipWithIndex.foreach {
          case (colCount, colIndex) =>
            val runningTotalCol = runningTotal(colIndex)

            val ranksHere: List[Long] =
              targetRanks.filter(rank => runningTotalCol < rank && runningTotalCol + colCount >= rank)
  ...
  private def findTargetRanksIteratively(
    ...
    sortedAggregatedValueColumnPairs.mapPartitionsWithIndex(
      //((value, colIndex), count)
      (partitionIndex: Int, aggregatedValueColumnPairs: Iterator[((Double, Int), Long)]) => {
        //(列号，该列在此分区中的排名)
        val targetsInThisPart: List[(Int, Long)] = ranksLocations(partitionIndex)._2

        if (targetsInThisPart.nonEmpty) {
          FindTargetsSubRoutine.asIteratorToIteratorTransformation(aggregatedValueColumnPairs, targetsInThisPart)
        } else {
          Iterator.empty
        }
      }
  ...
  def findQuantilesWithCustomStorage(
      valPairs: RDD[((Double, Int), Long)],
